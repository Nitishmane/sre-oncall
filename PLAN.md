...
| Model provider | **OpenAI** (`openai/gpt-5-6-luna`, the cheapest in the catalog at $0.20/$1.20 per 1M) via TrueForge model config, since 2026-08-26 | Switched from Anthropic Claude: the Anthropic account backing this project has no credit, and every call returns 400 *"credit balance is too low"* — the single blocker on ever completing a healing session. The provider is configuration, not code (`SRE_ONCALL_MODEL`'s provider half picks both the registered provider and the API-key env var), so the swap costs one env line. Anthropic and google-gemini remain drop-in alternatives. |
