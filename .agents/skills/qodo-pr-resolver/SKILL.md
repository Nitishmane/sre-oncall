...
- **Do NOT post summary or resolve threads yet** — store the fix summary for Step 9
...
### Step 8: (Deferred-only) Post summary and reply to comments

**Only if all issues were deferred** (no code changes): Post a summary comment immediately.

**REQUIRED for deferred-only runs:** After all issues have been reviewed and deferred (with no fixes applied), ALWAYS post a comment summarizing the actions taken.
...
**For deferred-only runs:**
- Post the summary comment
- Resolve inline threads (set `"unresolved": false` for all deferred issues)
- Proceed to Step 10

**For runs with applied fixes:** skip this step and proceed to Step 9.

### Step 9: Push to remote and post summary

**If any fixes were applied:**
1. Ask the user if they want to push: `git push` (for Gerrit: `git push origin HEAD:refs/for/<target-branch>`)
2. If push successful, **now post the summary comment and resolve threads:**
   - See [providers.md § Post Summary Comment](./resources/providers.md#post-summary-comment) for provider-specific commands and summary format.
   - **Round-of-record:** heading **must** be `## Qodo Fix Summary — Round N` with the verbatim `Generated by Qodo PR Resolver skill` footer.
   - **Gerrit:** Batch the summary comment AND all inline replies into a **single API call**. Use the unified review endpoint with both `message` (summary) and `comments` (inline replies) — see [gerrit.md § Post Summary Comment](./resources/gerrit.md#post-summary-comment).
   - **Important resolution rules for inline replies:**
     - **Fixed** issues: set `"unresolved": false` (resolves the thread)
     - **Held prior decision / hard-stopped**: set `"unresolved": false` and make the reply state the prior decision was deliberate. In the summary, list these under a **dedicated category** titled **"Skipped to prevent oscillation — recommend human resolution"** (NOT under Deferred), each with `file:line` and oscillation reason.
   - **After posting the summary, resolve the Qodo review comment:** Find the Qodo "Code Review by Qodo" comment and mark it as resolved or react to acknowledge it. See [providers.md § Resolve Qodo Review Comment](./resources/providers.md#resolve-qodo-review-comment) for provider-specific commands. If resolve fails, continue — the summary comment is the important part.
3. If push declined or fails, inform the user: "Changes are committed locally but not yet pushed. The remote PR does not yet contain these fixes. Please push when ready: `git push`"

**If all issues were deferred:** skip to Step 10 (deferred-only runs already posted in Step 8).
